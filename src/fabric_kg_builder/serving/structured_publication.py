...
L5A_PUBLICATION_CODE_VERSION = "0.2.3/l5a-1"
...
class _CallAccounting:
    ...
    def observe_operation(self, operation: L5aPublishOperation) -> None:
        self.network_request_bytes += operation.request_bytes
        self.network_response_bytes += operation.response_bytes
        self.retry_count += operation.retry_count
        self.retry_wait_ms += operation.retry_wait_ms
        self.remote_operation_refs = tuple(sorted({
            *self.remote_operation_refs,
            *operation.operation_refs,
        }))
...
def _validate_operation(operation: object) -> L5aPublishOperation:
    if not isinstance(operation, L5aPublishOperation):
        raise L5aPublicationError(
            ...
            "target adapter returned an unsupported operation result",
        )
    for name in (
        "request_bytes",
        "response_bytes",
        ...
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise L5aPublicationError(
                "L5A_OPERATION_INVALID",
                f"target operation {name} must be a nonnegative integer",
            )
    if (
        not isinstance(operation.created, bool)
        or not isinstance(operation.applied, bool)
        or not operation.publication_token
        or not isinstance(operation.operation_refs, tuple)
        or any(not isinstance(item, str) for item in operation.operation_refs)
    ):
        raise L5aPublicationError(
            "L5A_OPERATION_INVALID",
            "target operation contains malformed status or reference fields",
        )
    return operation
...
def _canonical_assertion_ids(compiled: L5aCompiledPublication) -> tuple[str, ...]:
    ids: set[str] = set()
    for table_id, table in compiled.tables.items():
        if not table_id.startswith("l4_semantic_asserted_"):
            continue
        field = {
            "l4_semantic_asserted_entities": "entity_id",
            "l4_semantic_asserted_relationships": "relationship_id",
        ...
        if field is None:
            continue
        kind = field.removesuffix("_id").removesuffix("_assertion")
        ids.update(f"{kind}|{value}" for value in table[field].to_pylist())
...
def _projection_evidence(
    ...
    ids = _canonical_assertion_ids(compiled)
    values: dict[str, Any] = {
        "count": len(ids),
        "canonical_id_set_hash": canonical_sha256(ids),
        "row_fingerprint": None,
        "definition_fingerprint": None,
        "index_fingerprint": None,
    }
    if kind == "parquet":
        values["row_fingerprint"] = canonical_sha256(
            [item.as_dict() for item in state.table_snapshots]
        )
    else:
        values["definition_fingerprint"] = canonical_sha256(state.definition)
...
def _equivalences(
    ...
    for crosswalk in compiled.crosswalks:
        for kind in L5A_TARGET_ORDER:
            state = states[kind]
            evidence = _projection_evidence(compiled, kind, state)
            values = {
                "identity": _identity(
                    compiled.source,
...
def _existing_is_intact(
    ...
    if (
        ...
        or not 12 <= metrics.fabric_calls <= 16
        or not (
    ...
    *,
    ...
    if existing is not None:
        ...
        try:
            ...
            for kind in L5A_TARGET_ORDER:
                accounting.observe_call()
                state = client.read_back(kind, compiled.target_ids[kind])
    ...
    try:
        ...
        for kind in L5A_TARGET_ORDER:
            accounting.observe_call()
            prior = client.inspect(kind, compiled.target_ids[kind])
        ...
        for kind in L5A_TARGET_ORDER:
            ...
            accounting.observe_operation(operation)
        ...
        for kind in publish_started:
            ...
            try:
                accounting.observe_call()
                recovered = client.inspect(kind, target_id)
                if (
...
def require_l5a_publication_receipt(
    ...
    *,
    ...
    fresh_skip_readback = result.receipt.status != "skipped"
    if result.receipt.status == "skipped" and client is not None:
        try:
            for kind in L5A_TARGET_ORDER:
                _validate_state(
                    client.read_back(kind, result.compiled.target_ids[kind]),
            ...
            fresh_skip_readback = False
        else:
            fresh_skip_readback = True
    skip_receipt_matches = (
        result.receipt.status == "skipped"
        and fresh_skip_readback
        ...
        and not result.receipt.remote_operation_refs
        ...
        and result.metrics.fabric_calls == 4
        ...
        and result.metrics.retry_count == 0
        and result.metrics.retry_wait_ms == 0
        ...
        and result.metrics.budget_snapshot_hash
        == canonical_sha256({
            "stage": "L5a",
            "bounded_target_calls": 17,
            "numeric_thresholds": None,
