...
## Scope: Deployment Manifest as Fabric Item Naming Authority (Issue #6, 2026-07-23)

- **Design review completed** on branch `scope/deploy-manifest`. ADR at `.squad/decisions/inbox/keyser-deployment-manifest.md`.
- **Root cause mapped:** Fabric item display names have FIVE competing authorities with no owner and no conflict detection — (1) env JSON dict reads (`fabric.lakehouse_display_name`, `ontology_display_name(_suffix)`, `data_agent_display_name`, `graph_model_display_name`), (2) Click `--option` defaults, (3) generated semantic title via `semantic/compiler.py:1161` `"name": ontology_name or contract_name` (the issue's silent-override), (4) generated `.platform` metadata `displayName` at `ontology/fabric_def.py:104-114` `_platform_part`, (5) command `--display-name`/`--graph-model-name`/`--index-name`. Naming logic is duplicated between `cli/deploy_cmd.py` standalone commands and `cli/build_deploy_cmd.py::_deploy_knowledge`.
- **Architecture decided:** NEW `deploy/manifest.py` (`DeploymentManifest` Pydantic + `load_deployment_manifest`, reusing `infra/manifest.py`'s `${ENV_VAR}` interpolation + structured error pattern) and NEW pure `deploy/name_authority.py` (`ResolvedName`, `resolve_item_name`, `validate_readback_name`, `render_name_resolution`, `manifest_from_env_config`, `NameAuthorityConflict`). Manifest is the single authority; generated-metadata/command mismatch ⇒ hard `NAME_AUTHORITY_CONFLICT`; legacy env fields are migration inputs only (warn, manifest wins).
- **Key constraints:** Distinguish new `DeploymentManifest` (Fabric item identity) from existing `InfraManifest` (Azure provisioning) — same loader pattern, different class/module, to avoid concept collision. Reuse `infra/names.py` `validate_fabric_identifier_name` / `validate_fabric_graph_model_name` — do NOT add validators. Error format follows `ERROR <CODE>:\n<msg>\n<remediation>` (`knowledge/validation.py:504`). Ontology dry-run is `--mock`, not `--dry-run` — inject resolved-name block into mock output; don't rename flags. Standardize rename/replace approval on existing `--approve-replace`.
- **Golden-hash risk flagged:** `semantic/compiler.py:1161` change must keep output byte-identical when manifest name == contract name; only enforce at deploy boundary.
