...
def test_provider_schema_requires_exact_root_or_child_identity_state() -> None:
    ...
    assert schema["properties"]["semantic_type_candidates"]["minItems"] == 0
    assert schema["properties"]["relationship_candidates"]["minItems"] == 0
    assert schema["properties"]["completeness_candidates"]["minItems"] == 0
    assert schema["properties"]["question_routes"]["minItems"] == 5
