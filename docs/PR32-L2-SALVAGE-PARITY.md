...
| `enrichment/output_schema.py` | Change dependency | C0 owns carrier schemas. L2 emits exact C0 `ExtractionCandidateBatch@1.0.0` and `RequiredMemberSetProposal@1.1.0` models and edits no contracts. |
...
| Required-member role/order defaults | Replace | C0 1.1 carries exact sealed ordering, approved roles, and nullable collection bounds; L2 emits null for absent role/order and never fabricates a sentinel or enumerate order. |
